	...
	<Card.Root class="mx-auto w-full max-w-sm">
		<Card.Header>
			...
			<Card.Description class="text-center"
				>The following exams are available to you.</Card.Description
			>
		</Card.Header>
		<Card.Content class="flex flex-col gap-4 items-center">
			{#each data.userAvailableExams as exam (exam.id)}
				<Button variant="outline" href="/select/selfadmitted/{exam.id}"
					>Take {exam.name} ({facilities[exam.facilityId]}) &rarr;</Button
				>
			{:else}
