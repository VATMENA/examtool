...
<div class="flex h-screen w-full items-center justify-center px-4">
	<Card.Root class="mx-auto w-full max-w-sm">
		<Card.Header>
			<Card.Title class="text-2xl text-center">Exam report: {data.student.name_first} {data.student.name_last}</Card.Title>
				...
				{#each data.pastExams as exam (exam.id)}
					<Alert.Root>
						...
						<Alert.Description>
							...
							{#if exam.isSubmitted}
								<Button class="text-foreground" href="/exam/{exam.ticketId}/{exam.id}/0?review" variant="outline">Review exam questions &rarr;</Button>
							{/if}
						</Alert.Description>
					</Alert.Root>
				{:else}
					<p class="text-center text-muted-foreground">{data.student.name_first} {data.student.name_last} has not taken any exams.</p>
				{/each}
			{/if}
		</Card.Content>
